...
type Code = { id: string; slug: string; title: string; contentType: ActiveContent["type"]; status: "draft" | "published" | "paused" | "deleted"; revision: number; content: ActiveContent; render: QrRenderConfig; publishedVersionId?: string | null; updatedAt?: string };
...
export function QrEditorView() {
  ...
  async function publish() { if (!code) return; setBusy(true); try { const saved = await saveCode(code.id, code.revision, { title, content, render }); const published = await publishCode(code.id, saved.revision); setCode({ ...saved, status: "published", publishedVersionId: published.version.id }); setNotice("已发布，新版本立即生效"); } catch (error) { setNotice(error instanceof Error ? error.message : "发布失败"); } finally { setBusy(false); } }
  ...
  return <ProjectShell><main className="tp-qr-editor">
    <header className="tp-qr-editor__header"><div><span className="index-label">07 / ACTIVE QR</span><h1>活码工作台</h1><p>内容可更新，二维码保持不变。</p></div><div className="tp-qr-editor__actions"><button type="button" className="button button--secondary" onClick={() => setNotice("预览地址：" + payload)}><Eye />预览</button><button type="button" className="button button--secondary" disabled={busy || !code} onClick={() => void saveDraft()}><Save />保存草稿</button><button type="button" className="button button--teal" disabled={busy || !code} onClick={() => void publish()}><Send />发布</button></div></header>
    {code ? <div className="tp-qr-editor__grid"><section><label className="tp-title-field"><span>项目名称</span><input value={title} onChange={(e) => setTitle(e.target.value)} /></label><ContentEditor value={content} onChange={setContent} onUpload={handleUpload} uploading={uploading} /></section><section className="tp-qr-preview"><div className="tp-qr-paper"><QRCodeCanvas data={payload} render={render} /></div><span className="tp-status tp-status--teal"><i />{code.status === "published" ? "已发布" : "草稿"} · revision {code.revision}</span><code>{payload}</code><div className="tp-download-row"><select aria-label="下载格式" value={format} onChange={(e) => setFormat(e.target.value as typeof format)}><option value="png">PNG</option><option value="svg">SVG</option><option value="webp">WEBP</option><option value="jpg">JPG</option></select><button type="button" className="button button--primary" disabled={busy} onClick={() => void exportQr()}>{typeof navigator.share === "function" ? <Share2 /> : <Download />}{typeof navigator.share === "function" ? "分享 / 下载" : "下载"}</button></div></section></div> : null}
