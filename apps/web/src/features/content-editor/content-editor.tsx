...
export function ContentEditor({ value, onChange, onUpload, uploading = false, fieldErrors = {} }: Props) {
  ...
  return <section className="tp-content-editor" aria-label="活码内容编辑器">
    ...
    {value.type === "image" && <><label><span>标题</span><input value={value.title} onChange={(e) => set("title", e.target.value)} />{fieldError("title")}</label><label><span>替代文本</span><input value={value.alt} onChange={(e) => set("alt", e.target.value)} />{fieldError("alt")}</label>{fieldError("assetId")}</>}
    {value.type === "video" && <><label><span>标题</span><input value={value.title} onChange={(e) => set("title", e.target.value)} />{fieldError("title")}</label><label className="tp-check"><input type="checkbox" checked={value.autoplay} onChange={(e) => set("autoplay", e.target.checked)} />自动播放（静音）</label><label className="tp-check"><input type="checkbox" checked={value.loop} onChange={(e) => set("loop", e.target.checked)} />循环播放</label>{fieldError("assetId")}</>}
    {value.type === "audio" && <><label><span>标题</span><input value={value.title} onChange={(e) => set("title", e.target.value)} />{fieldError("title")}</label><label><span>艺术家</span><input value={value.artist} onChange={(e) => set("artist", e.target.value)} />{fieldError("artist")}</label>{fieldError("assetId")}</>}
    {value.type === "file" && <><label><span>显示标题</span><input value={value.title} onChange={(e) => set("title", e.target.value)} />{fieldError("title")}</label><label><span>文件名</span><input value={value.downloadName} onChange={(e) => set("downloadName", e.target.value)} />{fieldError("downloadName")}</label><label><span>描述</span><textarea value={value.description} onChange={(e) => set("description", e.target.value)} />{fieldError("description")}</label>{fieldError("assetId")}</>}
    {value.type === "url" && <><label><span>安全网址</span><input type="url" value={value.url} onChange={(e) => set("url", e.target.value)} placeholder="https://example.com" />{fieldError("url")}</label><label><span>标题</span><input value={value.title} onChange={(e) => set("title", e.target.value)} />{fieldError("title")}</label><label><span>描述</span><textarea value={value.description} onChange={(e) => set("description", e.target.value)} />{fieldError("description")}</label></>}
    {value.type === "text" && <><label><span>标题</span><input value={value.title} onChange={(e) => set("title", e.target.value)} />{fieldError("title")}</label><label><span>正文</span><textarea maxLength={4000} value={value.text} onChange={(e) => set("text", e.target.value)} />{fieldError("text")}</label><small>{value.text.length}/4000</small></>}
    {value.type === "contact" && <div className="tp-contact-grid">{([["firstName", "名"], ["lastName", "姓"], ["organization", "组织"], ["title", "职位"], ["phone", "电话"], ["email", "邮箱"], ["website", "网站"], ["address", "地址"]] as const).map(([key, label]) => <label key={key}><span>{label}</span><input type={key === "email" ? "email" : key === "website" ? "url" : "text"} value={value[key]} onChange={(e) => set(key, e.target.value)} />{fieldError(key)}</label>)}<label><span>备注</span><textarea value={value.note} onChange={(e) => set("note", e.target.value)} />{fieldError("note")}</label></div>}
  </section>;
}
