...
import { CalendarDays, FileImage, Hash, Mail, Phone, Plus, Save, Send, Type } from "lucide-react";
...
import { fieldLibrary, initialBuilderFields, type BuilderField, type BuilderFieldType } from "../model/form-fields";
...
export function FormBuilderView() {
  ...
  useEffect(() => {
    void api.project(projectId).then(({ project: loaded }) => {
      ...
      if (schema) setFields(schema.fields.map((field) => ({ id: field.id, label: field.label, required: field.required, type: toUiType[field.type] })));
  ...
  function addField(type: BuilderFieldType) {
    setFields((currentFields) => [...currentFields, { id: crypto.randomUUID(), label: `新${type}`, type, required: false }]);
    setStatus("尚有未发布修改");
  }
  ...
  function schemaFields(): FormField[] {
    return fields.map((field) => ({ id: field.id.match(/^[0-9a-f-]{36}$/i) ? field.id : crypto.randomUUID(), type: toSchemaType[field.type], label: field.label, required: field.required }));
  }
  ...
  return (
    <ProjectShell>
      <section className="form-builder-view">
        ...
        <aside className="field-library paper-builder-sheet"><h3>字段库</h3>{fieldLibrary.map((type) => { const Icon = fieldIcon(type); return <button key={type} type="button" onClick={() => addField(type)}><Icon />{type}</button>; })}<button className="field-library__add" type="button" onClick={() => addField("短文本")}><Plus />添加字段</button></aside>

        <div className="builder-fields paper-builder-sheet">
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={fields.map((field) => field.id)} strategy={verticalListSortingStrategy}>
              {fields.map((field) => <SortableFieldRow key={field.id} field={field} onDuplicate={duplicateField} onRemove={(id) => setFields((currentFields) => currentFields.filter((item) => item.id !== id))} onRequiredChange={(id, required) => setFields((currentFields) => currentFields.map((item) => item.id === id ? { ...item, required } : item))} />)}
            </SortableContext>
          </DndContext>
        </div>

        <aside className="form-preview paper-builder-sheet"><header><h3>公共页面预览</h3><span>{status}</span></header><div className="form-preview__phone"><strong>设备巡检记录</strong><img src={generatedAssets.industrialCover} alt="工业设备巡检现场" /><p>请按照实际情况填写巡检内容，确保设备运行正常。</p>{fields.slice(0, 5).map((field) => <label key={field.id}>{field.label}{field.required ? <b>*</b> : null}<input placeholder={field.type === "日期" ? "选择日期" : `请输入${field.label}`} /></label>)}</div></aside>
      </section>
    </ProjectShell>
  );
}
